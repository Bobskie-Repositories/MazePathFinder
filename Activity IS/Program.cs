...
namespace Activity_IS
{
    ...
    class Program
    {
        ...
        /// </summary>
        public int[,] FindPath(int iFromY, int iFromX, int iToY, int iToX, bool bfs)
        {
            int iBeginningNode = iFromY * this.Cols + iFromX;
            int iEndingNode = iToY * this.Cols + iToX;
            return (Search(iBeginningNode, iEndingNode, bfs));
        }
        ...
        private int[,] Search(int iStart, int iStop, bool bfs)
        {
            const int empty = 0;
            ...
            else // Greedy Best First Search with QUEUE
            {
                ...
                for (int x = 0; x < 13; x++)
                {
                    for (int y = 0; y < 10; y++)
                    {
                        heuristic[x, y] = Math.Abs(eX - x) + Math.Abs(eY - y);
                    }
                }
                ...
                while (iFront != iRear)                 // while Q not empty	
                {
                    ...
                    int hTop = 999, hDown = 999, hLeft = 999, hRight = 999 ; // default heuristic val in directions
                    iTop = iCurrent - iCols;
                    iDown = iCurrent + iCols;
                    iLeft = iCurrent - 1;
                    iRight = iCurrent + 1;
                    ...
                    int[] array = { hTop, hDown, hLeft, hRight };
